...
//
//		// When/Then
//		mockMvc.perform(post("/orders/checkout").header("Authorization", authHeader)
//				.contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(orderRequest)))
//				.andExpect(status().isCreated()).andExpect(jsonPath("$.id").value(orderResponse.getId()))
...
//
//		// When/Then
//		mockMvc.perform(get("/orders/1").header("Authorization", authHeader)).andExpect(status().isOk())
...
//
//		// When/Then
//		mockMvc.perform(get("/orders/user/1").header("Authorization", authHeader)).andExpect(status().isOk())
...
//				.content(objectMapper.writeValueAsString(orderRequest))).andExpect(status().isUnauthorized())
...
//
//		// When/Then
//		mockMvc.perform(get("/orders/user/1").header("Authorization", authHeader)).andExpect(status().isUnauthorized());
//	}
//
//	@Test
