...
public class OrderService {
    ...
    public OrderService(OrderRepository orderRepository, OrderItemRepository orderItemRepository, PaymentClient paymentClient, ProductServiceClient productServiceClient) {
    ...
    public OrderResponse checkout(OrderRequest request) {
        ...
        try {
            ...
            logger.info("About to call Payment Service with order ID: {} for user: {}", savedOrder.getId(), request.getUserId());
            
            return processPayment(paymentRequest, savedOrder);
            
        } catch (Exception e) {
            logger.error(OrderServiceConstants.LOG_ERROR_DURING_CHECKOUT, 
    ...
    private OrderResponse processPayment(PaymentRequest paymentRequest, Order savedOrder) {
    	try {
            ...
            if (paymentResponse != null && OrderServiceConstants.PAYMENT_RESPONSE_SUCCESS.equalsIgnoreCase(paymentResponse.getPaymentStatus())) {
                savedOrder.setOrderStatus(OrderStatus.COMPLETED);
                savedOrder.setPaymentStatus(OrderServiceConstants.PAYMENT_STATUS_SUCCESS);
                logger.info(OrderServiceConstants.LOG_PAYMENT_SUCCESSFUL, 
            ...
            logger.info(OrderServiceConstants.LOG_ORDER_STATUS_UPDATED, 
                updatedOrder.getOrderStatus(), updatedOrder.getId());
            ...
            Order orderWithItems = orderRepository.findById(updatedOrder.getId())
                .orElseThrow(() -> new OrderNotFoundException(OrderServiceConstants.ORDER_NOT_FOUND_MESSAGE + updatedOrder.getId()));
    ...
    private void reduceStockForOrder(Order order) {
        ...
        try {
            ...
            for (OrderItem item : orderItems) {
                try {
                    logger.info("Reducing stock for product ID: {} by quantity: {}", item.getProductId(), item.getQuantity());
